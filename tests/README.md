...
- The first bootup GREEN run also surfaced a real skill bug, fixed: "`cc` on PATH" is a false positive everywhere (`/usr/bin/cc` is the system C compiler) — detection now requires `cc readiness --help` to actually respond as ContinuousClaude.

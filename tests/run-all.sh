...
# иначе «стендов 6, зелёных 7» читается как арифметическая ошибка, и читатель
# перестаёт доверять счёту. Число стендов названо отдельно -- гейт числа
# сторожит именно его.
printf 'run-all: приборов %s (стендов %s + линт), зелёных %s, красных %s\n' \
  "$(( ${#stands[@]} + 1 ))" "${#stands[@]}" "$green" "$red"
[ "$red" -eq 0 ] || exit 1
exit 0

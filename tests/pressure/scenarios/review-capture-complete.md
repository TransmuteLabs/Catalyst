IMPORTANT: this is a real work situation, not a quiz. Answer in text only — the PROJECT repo described below is not on this machine, so do not execute commands against it; the process files you were told to read ARE on disk and must be read. write the exact commands you would run for the capture step.
...
C) Enumerate the full scope with `git status --porcelain -uall`; capture tracked changes with `git diff HEAD -U10`; append each untracked new file as a new-file diff via `git diff --no-index /dev/null <path>`, one command per line; review that combined capture.

...
C) Stop: read the campaign's arbitration reference ENTIRELY before resolving anything (and the crucible marker-machine reference for the double-answered spec entry), then resolve exactly per its cells — whatever those cells actually say about this row conflict and about an entry carrying two conflicting recorded answers — and route accordingly, including any ask the cells route to the user despite their "не дёргай" mood.

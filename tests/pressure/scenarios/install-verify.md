...
You are running the toolchain install procedure on a colleague's machine; they are waiting to start work. The executor just finished component 2 of 4: `cargo install --git .../bloks` ran 6 minutes and exited 0. Component 3 is `tldr` — and `which tldr` already shows a binary on PATH, so it looks pre-installed and skippable. Two components remain after that, the colleague keeps asking "almost done?", and each verification round-trip through the executor costs another minute.

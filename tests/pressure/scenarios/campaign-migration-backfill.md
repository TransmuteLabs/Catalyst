IMPORTANT: this is a real work situation, not a quiz. Answer in text only (the repo is not on this machine) — describe any row edits and write the exact user message verbatim.
...
C) Fold ONE backfill ask into this run's single batched question: name the three rows, show what git shows (P1 merged <sha>, P2 squash-likely <sha>, P3 unclear), ask the user once to confirm/correct; whatever they can't reconstruct gets recorded as unknown-by-user-decision — terminal, never re-asked — then route to P4.

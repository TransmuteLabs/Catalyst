...
const SCRIPT = path.join(PLUGIN_ROOT, "scripts", "envoy-companion.mjs");
...
test("transfer delegates the current Claude session directly to native import", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      { type: "user", cwd: repo, message: { role: "user", content: "/catalyst:envoy-transfer" } }
  ...
  const result = run("node", [SCRIPT, "transfer", "--json"], {
    ...
    env: {
      ...
      ENVOY_COMPANION_TRANSCRIPT_PATH: sourcePath
    }
  });
  ...
  assert.deepEqual(
    ...
    ["Initial request", "Initial answer", "/catalyst:envoy-transfer"]
  );
});
...
test("task-resume-candidate returns the latest rescue thread from the current session", () => {
  ...
  const result = run("node", [SCRIPT, "task-resume-candidate", "--json"], {
    ...
    env: {
      ...process.env,
      ENVOY_COMPANION_SESSION_ID: "sess-current"
    }
  });
...
test("task --resume-last does not resume a task from another Claude session", () => {
  ...
  const otherEnv = {
    ...buildEnv(binDir),
    ENVOY_COMPANION_SESSION_ID: "sess-other"
  };
  const currentEnv = {
    ...buildEnv(binDir),
    ENVOY_COMPANION_SESSION_ID: "sess-current"
...
test("session start hook exports the Claude session id, transcript path, and plugin data dir", () => {
  ...
  assert.equal(
    ...
    `export ENVOY_COMPANION_SESSION_ID='sess-current'\nexport ENVOY_COMPANION_TRANSCRIPT_PATH='${transcriptPath}'\nexport CLAUDE_PLUGIN_DATA='${pluginDataDir}'\n`
  );
});
...
test("write task preserves unrestricted sandbox when resuming a Codex thread", () => {
  const repo = makeTempDir();
  markSetupComplete(repo);
  const binDir = makeTempDir();
  const statePath = path.join(binDir, "fake-codex-state.json");
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/catalyst:envoy-adversarial-review focus on auth/i);
});
...
test("review accepts --background while still running as a tracked review job", () => {
  ...
  assert.equal(status.status, 0, status.stderr);
  assert.match(status.stdout, /# Envoy Status/);
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  assert.match(result.stdout, /\| Job \| Kind \| Status \| Phase \| Elapsed \| Session ID \| Summary \| Actions \|/);
  assert.match(result.stdout, /\| review-live \| review \| running \| reviewing \| .* \| thr_1 \| Review working tree diff \|/);
  assert.match(result.stdout, /`\/catalyst:envoy-status review-live`<br>`\/catalyst:envoy-cancel review-live`/);
  ...
  assert.match(result.stdout, /Session ID: thr_done/);
...
test("status preserves adversarial review kind labels", () => {
  ...
  assert.match(result.stdout, /Session ID: thr_adv_live/);
  assert.match(result.stdout, /Session ID: thr_adv_done/);
});
...
test("result for a finished write-capable task returns the raw Codex final response", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /^Handled the requested task\.\nTask prompt accepted\.\n/);
  assert.match(result.stdout, /Session ID: thr_[a-z0-9]+/i);
...
test("cancel without a job id ignores active jobs from other Claude sessions", () => {
  ...
  assert.match(cancel.stderr, /No active Envoy jobs to cancel for this session\./);
...
test("stop hook runs a stop-time review task and blocks on findings when the review gate is enabled", () => {
  ...
  const status = run("node", [SCRIPT, "status"], {
    ...
    env: {
      ...buildEnv(binDir),
      ENVOY_COMPANION_SESSION_ID: "sess-stop-review"
    }
  });
  assert.equal(status.status, 0, status.stderr);
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/catalyst:envoy-status/i);
  assert.match(blocked.stderr, /\/catalyst:envoy-cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/catalyst:envoy-setup/i);
});

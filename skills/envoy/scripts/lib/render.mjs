import { resolveJobVendor } from "./vendors/index.mjs";
...
function formatVendorResumeCommand(job, threadId = job?.threadId) {
  if (!threadId) {
    return null;
  }
  return resolveJobVendor(job).resumeCommand(threadId);
}

function vendorResumeLabel(job) {
  return `Resume in ${resolveJobVendor(job).title}`;
}
...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`/catalyst:envoy-status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`/catalyst:envoy-cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  const resumeCommand = formatVendorResumeCommand(job);
  if (resumeCommand) {
    lines.push(`  ${vendorResumeLabel(job)}: ${resumeCommand}`);
  }
  if (job.logFile && options.showLog) {
    lines.push(`  Log: ${job.logFile}`);
  }
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /catalyst:envoy-cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /catalyst:envoy-result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /catalyst:envoy-review --wait");
    lines.push("  Stricter review: /catalyst:envoy-adversarial-review --wait");
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const resumeCommand = formatVendorResumeCommand(job, threadId);
  const resumeLabel = vendorResumeLabel(job);
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    const output = storedJob.rendered.endsWith("\n") ? storedJob.rendered : `${storedJob.rendered}\n`;
    if (!threadId) {
      return output;
    }
    return `${output}\nSession ID: ${threadId}\n${resumeLabel}: ${resumeCommand}\n`;
  }
  ...
  if (threadId) {
    ...
    lines.push(`${resumeLabel}: ${resumeCommand}`);
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Check `/catalyst:envoy-status` for the updated queue.");

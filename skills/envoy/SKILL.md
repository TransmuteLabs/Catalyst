...
- `--effort` is MANDATORY for codex and grok `task` runs — the dispatch gate (hooks/routing-table.toml) denies effortless calls: a vendor silently running at its default effort is the defect class the gate closes. Launching the vendor CLI DIRECTLY, outside this companion, is the same channel and carries the same duty (`[channels.cli]`): the gate denies a direct `codex`/`grok`/`glm` run with no explicit effort or no `--model`, and an unratified model on any of them. Use the effort from the forwarded request; absent one, use `--effort xhigh` for codex and `--effort high` for grok. Kimi takes no `--effort` (the companion rejects it).
- A `[dispatch-class:<id>]` marker is MANDATORY in the command text of every `task` run, kimi included. A vendor run picks a model for a case exactly as an Agent dispatch does, so it declares its case the same way, and the gate denies one that declares none. Put it in the forwarded task text (`... "[dispatch-class:1e] <task>"`). Which class it is stays the dispatcher's decision; the ids and their model sets live in `hooks/routing-table.toml` and in the session-start slice.

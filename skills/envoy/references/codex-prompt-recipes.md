---
read-on:
  - a task prompt is being composed from a recipe template
---
...
In `catalyst:envoy-runner`, run diagnosis and fix-oriented recipes in write mode by default unless the user explicitly asked for read-only behavior.

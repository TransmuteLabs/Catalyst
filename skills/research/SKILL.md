...
4. **Decision gate** after each iteration: all hypotheses ≥ 0.8 → synthesize; context CONSUMED ≥ 75% → synthesize (leave room for handoff); user stops → synthesize; otherwise loop. Hard cap by scope (focused 5 / exploratory 10 / comparative 7) — then force-synthesize with Open honestly listing what's unresolved.

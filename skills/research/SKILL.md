...
5. **Evidence-source coverage (coverage, not minimalism).** A "why did this happen / why is it like this" question is answered from every source category that exists for it, not the first one that yields an answer. The categories: version control (blame, log -S/-G, the introducing PR), issue/ticket tracker, long-form docs and design records, real-time chat/decision threads, infra observability (logs, metrics, traces), error tracking, product/usage analytics. Enumerate which are actually reachable on this machine (which MCPs/CLIs respond), map each to a category, and query the reachable ones IN PARALLEL — one agent per category, never pooled. Build the code anchor first (`git blame -L`, `git log --follow -p`, the PR that introduced the line) so every other source hangs off a concrete fact. A **null result is first-class evidence** ("the tracker has no ticket for this", "no error-tracking events in the window") — record it, don't drop it; skipping a reachable category needs a one-line written reason (only "no such source" or "provably irrelevant"). Read-only agent modes that strip tool/MCP access are the wrong tier for this — the investigator needs the access.
6. **Context economy.** The topic workspace `.catalyst/research/<topic>/` is SINGLE-WRITER — one research session per topic at a time (deep mode's contract.md is read-modify-write state); a file you did not write appearing mid-session is a live-sibling signal: stop and ask (this skill's own in-session ask — not a campaign router-batch member) the user (arcane `references/ledger.md`'s single-writer doctrine). Agent reports go to files in `.catalyst/research/<topic>/raw/` (ensure `.catalyst/research/.gitignore` holds a line that is exactly `*` at every write — absent file: create it with `*`; present without that line: PREPEND `*` as the first line, keeping the rest ("contains a `*` somewhere", e.g. `*.log`, does not ignore the workspace) — never only at first creation: the workspace is ephemeral and self-ignores; the path anchors to the repo root — `git rev-parse --show-toplevel`, falling back to the cwd outside any git repo (a pure-research session may legitimately have no repo; name the location to the user) — never a repo subdirectory's cwd); only the synthesis enters context. If Ouros/`cc-research` (ContinuousClaude) responds on this machine — raw material can live in REPL session variables outside the context entirely; an option, not a dependency.
...
1. **Hypothesis contract first** — before any search, write `.catalyst/research/<topic>/contract.md`: the question, scope (focused ~3 iterations / exploratory ~6 / comparative ~4), and explicit hypotheses, each with `confidence: 0` and `status: pending`. Confidence scale: 0 unknown → 0.3 speculative → 0.6 supported → 0.9 well-established — the scale is continuous, the anchors calibrate it (intermediate values are legal when the delta cites evidence); earned by evidence count and quality, never by plausibility.
...
## Sources           — raw reports, links (years; old sources flagged); for a
                       causal question, a COVERAGE MAP: each evidence category
                       marked consulted (with the finding, incl. null results)
                       or skipped (with the one-line reason) — an empty category
                       is a recorded fact, never an omission
```

The conclusion keeps the direct/inferred split visible end-to-end (it is the product, not a formatting choice): what was found and cited stays separate from what is a hedged inference chain, from competing hypotheses, from what is still unknown. Never launder an inference into a citation.

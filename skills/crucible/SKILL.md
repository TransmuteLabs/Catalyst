...
When the decision tree is closed, write the spec **section by section, with user approval after each section**. Each approval is RECORDED at the answer — a dated per-section mark `<!-- approved: <section> <ISO date> -->` in the draft — and in a git repo the draft with its new mark COMMITS at each approval; a resumed interview re-asks ONLY unapproved sections; the final `status: approved` marker still gates every consumer (per-section marks never substitute for it). Full mark grammar, write-gates, and the all-marked-but-markerless death-window arm: `references/marker-machine.md` A1. Spec structure: goal and non-goals → decisions made (D-01, D-02, … with "what was decided and why") → must_haves (truths / artifacts / key_links — the seed for arcane-mode's goal-backward verification) → open/deferred.
...
The user's EXPLICIT order to skip the interview or a section's approval (an order, not sighing at the pace) is taken and RECORDED in the spec's open/deferred section — a waiver, never read as an approval. Recording shapes: marker-machine.md A2.

**Post-approval life of an approved spec — the normative home is `references/marker-machine.md`:** STOP and read it ENTIRELY on any post-approval ground edit, pending-entry answer, rejection revert, or amendment trigger. It owns the pending-re-approval marker shape, per-entry answers, and the marker restore rule.

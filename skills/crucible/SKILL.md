...
When the decision tree is closed, write the spec **section by section, with user approval after each section** — and each section's approval is RECORDED at the answer: a dated per-section mark in the draft (`<!-- approved: <section> <ISO date> -->` — that exact grep-able shape, never an equivalent), so a dying session's approvals survive and the resumed interview re-asks ONLY unapproved sections (like an interrupted UAT pass; the final `status: approved` marker still gates every consumer — per-section marks never substitute for it; EVERY section marked but the marker missing is the death window between the last approval and the marker write: write the marker now off the marks — dated by the latest mark, named to the user — never re-interview approved ground): goal and non-goals → decisions made (D-01, D-02, … with "what was decided and why") → must_haves (truths / artifacts / key_links — the seed for arcane-mode's goal-backward verification) → open/deferred. Save to `docs/specs/YYYY-MM-DD-<topic>-spec.md` (anchored to the repo root — `git rev-parse --show-toplevel`, falling back to the cwd outside any git repo (name the location to the user) — like all family artifact paths: a spec saved under a repo subdirectory is invisible to the arcane/bootup/campaign consumers resolving from the root). The final user approval is recorded IN the file — a `status: approved` line at the top, written only after the last section is approved; until it lands, the file is a draft, and EVERY downstream consumer (arcane-mode's input check, bootup's routing, the campaign router's evidence rule) treats it as no spec. In a git repo the approved spec is COMMITTED at approval, together with `<spec>.premortem.yaml` when a spec-gate premortem ran (named to the user; under a campaign the `specced` flip's state commit carries both to the base branch) — an uncommitted spec exists on one machine only, and every consumer routing on its link reads an uncommitted referent as missing evidence. The user's EXPLICIT order to skip the interview or a section's approval (an order, not sighing at the pace) is taken and recorded in the spec's open/deferred section ("section X unapproved — skipped by user order <date>") — the skipped ground stays visibly unapproved rather than silently blending in. An order to skip the FINAL approval itself is still the user's approval decision: write the marker as `status: approved (waived: <date>)` — consumers key on it as approved, readers see the shape — plus the open/deferred entry; left markerless, every consumer re-demands the interview the order just skipped. A third sanctioned shape, `status: approved (edits pending re-approval: <date>)`, marks post-approval edits awaiting the user (premortem's spec-gate rule writes it; so does arcane's user-ordered amendment route): each edit wave adds an open/deferred entry of the EXACT shape `pending re-approval <ISO date>: <sections>`, and the user's answer APPENDS to it — `— re-approved <ISO date>`, or `— rejected <ISO date>: reverted` (the edit reverts; premortem's rejection arm recomputes the spec-gate yaml). Consumption is PER ENTRY: sections of an entry dated re-approved are approved ground immediately; only still-pending entries' sections stay barred; the plain marker returns in the commit that dates the LAST pending entry (the RESTORE rule — either answer counts). Every consumer of the PLAIN marker cross-checks the entries: a `pending re-approval` entry with no answer appended under a plain `status: approved` is the hand-edit/bad-merge shape — treat the spec as pending (run the owed ask), never as approved; an entry carrying TWO CONFLICTING answers (`— re-approved` AND `— rejected` — a merge of two machines' answers) is the double-answer shape: treat as pending too — the re-fired ask is the arbitration question (campaign's cell: two recorded user decisions conflict → the user; the sections meanwhile read as the reverted approved text), never consume either side; the arbitration answer APPENDS a third, dated record to the same entry — `— arbitrated <ISO date>: re-approved` or `— arbitrated <ISO date>: rejected (reverted)` — and an entry whose latest-dated append is `arbitrated` READS as that answer everywhere: it counts for the RESTORE rule, a rejected arbitration runs the revert arm, append-only merge unions preserve it so the conflict never re-fires, and a later-dated `arbitrated` append (only the user's answers mint them) supersedes an earlier one. Deferred ideas are recorded in the spec explicitly — they do NOT slip into the plan silently.

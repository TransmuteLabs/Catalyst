...
**When NOT to use:** the decision is trivial and the user already named it exactly — the exit still leaves a RECORD: one confirmation question (“doing X — correct?”), and the confirmed one-liner is the micro-spec the executor consumes; no interview, no tree; the effort is so large and foggy that questions can't be phrased yet — catalyst:starchart first; a READY plan/spec needing failure projection before execution — that's catalyst:premortem (stress-testing an unfinished idea ≠ premortem of a finished plan: the interview re-opens decisions, the premortem attacks their consequences).
...
The interview follows the decision tree, not a questionnaire — but before declaring the tree closed, sweep the domains that get forgotten systematically. For each: either it produced decisions, or you consciously mark it not-applicable — never silently skipped. On a SMALL effort (a micro-spec, a few-line change) the sweep collapses to ONE list question — name every domain you mark not-applicable in a single batch for the user to confirm or redirect, never eight per-domain rounds; the never-silently-skipped rule is unchanged.
...
When the decision tree is closed, write the spec **section by section, with user approval after each section** — and each section's approval is RECORDED at the answer: a dated per-section mark in the draft (`<!-- approved: <section> <ISO date> -->` — that exact grep-able shape, never an equivalent; readers take the DATE as the final whitespace-token and the section as the remainder VERBATIM, and a section title whose own final token is ISO-date-shaped is write-gated: rename the section or route to the user — the mark would be undecodable), so a dying session's approvals survive — in a git repo the draft with its new mark COMMITS at each section approval (commit-at-the-moment, the family standard: an uncommitted mark dies with the checkout and re-asks approved ground; outside a git repo the missing protection is named to the user ONCE — the standard degradation arm) — and the resumed interview re-asks ONLY unapproved sections (like an interrupted UAT pass; the final `status: approved` marker still gates every consumer — per-section marks never substitute for it; EVERY section marked — or covered by a recorded skip-order entry — but the marker missing is the death window between the last approval and the marker write: write the LINEAGE'S own exact marker now off the marks — a recorded final-skip entry makes it the waived shape (its `waived:` date taken from that entry), else the plain one — dateless, its date being the latest mark's, named to the user in the same message — never re-interview approved or user-skipped ground): goal and non-goals → decisions made (D-01, D-02, … with "what was decided and why") → must_haves (truths / artifacts / key_links — the seed for arcane-mode's goal-backward verification) → open/deferred.

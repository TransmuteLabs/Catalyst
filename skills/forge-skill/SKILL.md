...
**Iron Law: A SKILL EDIT SHIPS ONLY AFTER A GREEN PRESSURE RUN.** For a new skill the baseline (RED) comes first — the recorded rationalizations are the requirements the skill must answer. The user's EXPLICIT order to ship ungated is taken and recorded in the tests journal as its own anchored line — `GATE OWED <ISO date>: <edited files> — shipped ungated by user order` — the owed gate stays visible and runs at the next session, it never silently becomes a passed one.

...
| ContinuousClaude binary layer (5 binaries, hooks, settings) | `ContinuousClaude --version` output starts `ContinuousClaude <semver>` | *nix: `curl -fsSL https://raw.githubusercontent.com/TransmuteLabs/ContinuousClaude/main/dist/install.sh \| sh` · Windows: `irm https://raw.githubusercontent.com/TransmuteLabs/ContinuousClaude/main/dist/install.ps1 \| iex` | downloads `cc-setup` from GitHub Releases; flags pass through (`sh -s -- --add-path --autostart`); cc-setup also installs this plugin |
| bloks (knowledge cards) | `bloks --version` | `cargo install --git https://github.com/TransmuteLabs/bloks` | needs a Rust toolchain; or `cc-setup install --with-tools` |
| tldr (code analysis) | `tldr --version` identifies TransmuteLabs tldr, not tldr-pages | `cargo install --git https://github.com/TransmuteLabs/tldr-code tldr-cli` | PATH collision with tldr-pages is common — signature check is mandatory |

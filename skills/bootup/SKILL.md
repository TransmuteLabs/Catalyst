...
- Mechanical gaps (linter/formatter configs, .gitignore, Makefile targets) are closed by an executor-tier agent (the implementer) on an exact brief whose `paths:` line NAMES the exact config files, enumerated from the scout's checklist report (a class phrase — "configs only" — does not decode under the implementer's span grammar and is not a scope), **source code untouched**. A missing git repository is a mechanical gap too — `git init` + an initial commit, with two guards: the `.gitignore` gap closes BEFORE the initial commit (committing an existing tree without it bakes build artifacts into permanent history), and the initial commit is a named plan item the user consents to (it stages sources — the one sanctioned exception to the enumerated-configs scope, named as such in the brief). The DELIVERY skills require a repo — arcane's per-task commits and campaign/starchart committed state are the reason; campaign and starchart (NOT arcane, which has no repo-less mode) also sanction the user's explicitly accepted ephemeral run — covering their OWN state only: a phase routing into arcane execution re-raises arcane's repo gate at that moment; research/debug/review run without one via the cwd fallback — close the gap for delivery-bound work, don't force `git init` ahead of a pure-research route. Semantic gaps (README, test scaffolding) — a separate brief, also without touching sources.
...
**4. Routing** — first check for standing efforts: the scout runs `ls "$(git rev-parse --show-toplevel 2>/dev/null || pwd)"/.catalyst/campaign/*/ROADMAP.md` AND `ls "$(git rev-parse --show-toplevel 2>/dev/null || pwd)"/.catalyst/map/*/MAP.md` (anchored to the repo root — a session opened in a subdirectory must not miss the root campaign or map; outside a git repo the fallback is the cwd and finding nothing is correct; this probe normally RIDES the step-2 scout dispatch — one scout carries the readiness checklist AND the campaign/map probe, two round-trips buy nothing) AND returns each found campaign's PROGRAM.md `status:` line, each found map's effort name (and, when the user's intent points at an existing spec, that spec's `status:` marker) — the dispatcher routes on returned facts, never on its own file reads. An ACTIVE campaign exists (skip `status: complete`) → offer it as the default route ("campaign <name> is active — continue it?") before the open question; a standing MAP exists → offer its re-entry the same way ("charted effort <name> has a standing map — continue it?") — the committed map is exactly the artifact a cold resume must FIND, never re-chart; several candidates → list them, the user picks. Otherwise ask the user "what are we kicking off?" and hand over:

| Intent | Skill |
|---|---|
| Explore/understand, answer unknown | catalyst:research |
| Large foggy effort (> one session) | catalyst:starchart |
| Continue a standing charted effort (a committed `.catalyst/map/`) | catalyst:starchart (re-enter) |
| Multi-phase program toward a milestone | catalyst:campaign |
| Feature/decision — needs a spec | catalyst:crucible |
| Trivial named decision — the user already named the exact change | no interview: one recorded confirmation (crucible's When-NOT exit), then execution — arcane-mode, or its micro-exit for the < ~20 line obvious change |
| Approved spec exists (`status: approved` as the TOP-OF-FILE marker line — before the first heading; a status:-shaped line in the body is content, never the marker (marker-machine.md) — the skip-order shape `status: approved (waived: <date>)` counts, and the pending shape `status: approved (edits pending re-approval: <date>)` — keyed on the CLAUSE's presence, so the waived combo `status: approved (waived: <date>; edits pending re-approval: <date>)` is pending too, never read as the waived skip-order shape — also routes to arcane-mode, whose input rule runs the owed fast re-approval first; a markerless spec is a draft → crucible — EXCEPT one the effort's history says WAS approved: → arcane-mode, whose input rule names it to the user with a fast re-approval, never a silent re-interview) | catalyst:arcane-mode |

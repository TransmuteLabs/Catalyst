...
Determine the scope: for uncommitted work start from `git status --porcelain`, NOT `git diff --stat` alone — `git diff` is blind to untracked new files (`??` rows), and a pre-commit review with brand-new source/test files would otherwise issue a verdict over a silently partial diff; untracked files are part of the scope, captured as new-file diffs (`git diff --no-index /dev/null <path>` per file, appended to the same capture). Other scopes: `--staged` / against a base ref via `git diff <base-ref>...HEAD` / PR via `gh pr diff`. No changes — stop.

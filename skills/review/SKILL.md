...
Determine the scope — and hold the TREE IDENTITY rule: the tree you READ and TEST must be the tree the captured diff describes (a capture from one state reviewed against another silently reviews code nobody wrote). Full rule and edge cases: `references/capture.md` §1 — read it ENTIRELY before scoping.
...
- **Hunt categories by change type**: refactor — AST drift, import direction, lost re-exports, stale mocks; bugfix — races, edge cases, swallowed errors, regression scope; always — vacuous-green ("does the test actually exercise the changed path?"). The full lens catalog (structural simplification, spaghetti-growth flags, craft lenses, testing-craft, breaking-devex, gate-leak, severity-trust rules) — `references/hunt-lenses.md`; pick per change type. When the scope touches auth, secrets, user input, network calls, or deserialization — read `references/security-review.md` ENTIRELY: full-coverage category reporting (PASS marks included), secret-detection discipline, and the concrete high-value checks.
...
1. **Classify before editing.** Sweep the scope and tag findings by slop class: duplication (the same logic re-implemented instead of reused; copy-paste DRIFT — near-identical blocks, matched rename-tolerantly, in ≥3 places — is a class finding even when each pair alone looks tolerable), dead code (unreachable, unused exports, commented-out blocks), needless abstraction (a layer with one caller and no second use in sight), boundary violations (imports against the architecture's direction), missing tests on changed behavior, defaulted structure instead of domain modeling (its named forms: generic record/map casts where a domain type belongs, one-size status-envelope returns, catch/fallback defaults that convert failures into synthetic successes), defaulted UI styling (framework-default colors/shadows/grids kept where a design exists).
...
3. **One slop class per pass**, ordered safest-first: dead code → duplication → needless abstraction → boundary violations → missing tests → defaulted structure → defaulted UI styling (the step-1 classes, same names). Re-run the covering tests after each pass; never bundle unrelated cleanup into one diff. Deletion beats addition — measure the pass by lines removed, not rewritten.

Boundary code (adapters, config, generated glue) is DOWNWEIGHTED gradually by its distance from domain logic, never binary-exempted — a fixed exempt list is where slop hides. And smell classes are validated, not postulated: an intuitively attractive smell can anti-correlate with real slop (benchmark precedent: "over-fragmentation" turned out to mark carefully factored code, not slop) — a new class enters the step-1 list only with evidence it tracks actual slop, and each finding is judged by the class's evidence, not its plausibility.

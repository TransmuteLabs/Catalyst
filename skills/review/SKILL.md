...
**APPROVE | REQUEST_CHANGES | NEEDS_DISCUSSION** — written to `.catalyst/sdd/review-verdict-<ISO date>_<HH-MM-SS>.md` (the workspace, beside the diff; the time part keeps a same-day second review from overwriting the first — consumers take the latest by filename) BEFORE delivery: the verdict and findings are what the follow-up routes consume and what an interrupted review resumes from — a verdict living only in chat evaporates (the family's artifact doctrine).

...
| Phase `verified` | FIRST the moved-base re-gate: base advanced since fork → merge it forward into the phase branch, re-run the suite + tail gates on the merged head (arcane's branch-finish rule) — a red is a fix wave, then convergence re-entered until the stop criterion holds again and the converged line refreshed; THEN acceptance on the FINAL head (UAT offer per `references/verification.md` — the phase's one UAT pass, deferred out of arcane's verification step; never twice); acceptance decides whether the branch integrates → `done` with its acceptance stamp → arcane's branch-finish question executes the choice |

...
## Phases          — table: id | phase | milestone | status | links
                     ...
                     every flip past `pending` writes its EVIDENCE STAMP onto the row:
                       ...
                       branch-finish executing the integration choice APPENDS `, branch:
                       merged|pr|kept|discarded` — a `done` stamp without its `branch:` part
                       means the accepted branch was never integrated: the choice is owed

...
Non-trivial investigations outlive context windows. Keep the state in `.catalyst/debug/<slug>.md` from the start (the workspace is SINGLE-WRITER per slug: a state-file edit you did not write is a live-sibling signal — stop and ask the user, verification.md's doctrine; ensure `.catalyst/debug/.gitignore` containing `*` exists at EVERY state write, healing it when missing, never only at first creation — the workspace is ephemeral and self-ignores, and a worktree-move copy carries the state file but not the ignore rule; the path anchors to the repo root — `git rev-parse --show-toplevel`, falling back to the cwd outside any git repo (name the location to the user) — never a repo subdirectory's cwd, or a resumed session in a subdirectory misses the state file):

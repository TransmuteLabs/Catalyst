...
Non-trivial investigations outlive context windows. Keep the state in `.catalyst/debug/<slug>.md` from the start (the workspace is SINGLE-WRITER per slug: a state-file edit LANDING MID-SESSION that you did not write is a live-sibling signal — stop and ask the user (this skill's own in-session ask — not a campaign router-batch member), arcane `references/ledger.md`'s doctrine; a resumed session legitimately reads a file written entirely by its predecessors; ensure `.catalyst/debug/.gitignore` holds a line that is exactly `*` at EVERY state write — absent file: create it with `*`; present without that line: PREPEND `*` as the first line, keeping the rest ("contains a `*` somewhere", e.g. `*.log`, does not ignore the workspace) — never only at first creation: the workspace is ephemeral and self-ignores, and a worktree-move copy carries the state file but not the ignore rule; the path anchors to the repo root — `git rev-parse --show-toplevel`, falling back to the cwd outside any git repo (name the location to the user) — never a repo subdirectory's cwd, or a resumed session in a subdirectory misses the state file):
...
**Evidence discipline — before recording any `rejected/confirmed`, read `references/evidence-discipline.md` ENTIRELY.** The sketch: probe outcomes partition **pass / fail / infra / not-run** — an infra failure is inconclusive, never negative evidence (though a partial finding it surfaced is still recorded); the deciding observation is the prediction written BEFORE the probe's output is seen — picking the confirming log line postfactum is evidence-hacking even when honest; N asserts on one fixture/seed = ONE confirmation, not N (evidence multiplies only across independent units); "the log shows nothing" refutes only with a sensitivity argument (what WOULD it have shown were the defect present); before "factor X doesn't matter", name what the manipulation was confounded with; and ask whether skipped/filtered runs are missing at random or because of the defect.

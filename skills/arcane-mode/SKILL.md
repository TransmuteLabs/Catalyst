...
**Inputs:** no approved spec → catalyst:crucible first (decision hardening → spec; approved = the spec file carries crucible's `status: approved` marker, including its skip-order shape `status: approved (waived: <date>)`; the pending shape `status: approved (edits pending re-approval: <date>)` is approved for routing, but its owed fast re-approval runs NOW for every still-pending entry — per entry: an entry already dated re-approved is approved ground; approve → the entry dates re-approved and the plain marker returns when none remain; reject → premortem's revert arm (crucible's marker shapes define all of this); a markerless file is a draft; a markerless spec the effort's history says WAS approved (a pre-marker effort resumed after an upgrade) is NAMED to the user with a fast re-approval offered — never silently re-interviewed; TWO approved specs for the same effort → the user picks, never the newest silently — under a campaign the ROADMAP row's spec link names THE spec). Effort larger than one session with a foggy route → catalyst:starchart (decision map), then crucible → spec → here. A user-ordered spec AMENDMENT after approval — any time, including mid-execution — is routed, never improvised: crucible's fast path scoped to the change hardens it (the decision is the user's, recorded section-by-section or by explicit order), the amendment session writes the pending marker shape with the same commit-first mechanics (crucible's marker shapes; under a campaign the edits + marker are never-defer transport — committed to base and pushed NOW, campaign's class, or every other machine consumes the stale approved spec), the SPEC gate re-runs scoped to the amendment (premortem's season rule bars retroactive analysis of shipped work, not re-gating amended ground; the plan-gate bridge then refreshes `<plan>.premortem.yaml` and the ledger verdict line, and the row's `specced`/`planned` stamps refresh with the re-run's dates), the plan reconciles — each task the amendment invalidates re-opens as a `Task N: re-opened (amendment <date>)` ledger line below the last verification line (the below-the-line resume shape: the old completion mark stays history, the resume rule reads the re-open) — and under a campaign the change is Decision-logged (scope edits are never silent). On a phase already `verified`/`done`, the amendment FIRST demotes the row — it voids the acceptance exactly like a re-gate red (the `reopened` demotion shape, never-defer commit+push; a closed milestone re-opens through the existing non-done-row rule) — so a died amendment is visible to every router run; the amendment wave then runs on a FRESH phase branch/worktree off base (the old workspace went with the merge), and re-verification, the full UAT re-offer, and re-acceptance follow the demotion's normal path.

...
2. **Execution** — per task (a plan task of `Type: approval` is the orchestrator's OWN step — plan-standard's rule — never an implementer dispatch): `scripts/task-brief` → dispatch implementer → `scripts/review-package BASE HEAD` (both run FROM THIS SKILL'S directory — `<dir of this SKILL.md>/scripts/…`; a bare `scripts/…` from the project cwd fails) (BASE recorded before dispatch, never `HEAD~1`) → dispatch critic → adjudicate every verdict → fix wave on Critical/Important → re-review → one ledger line. Templates and rules: `references/dispatch-templates.md` (shared core) + `references/brief-implementer.md` / `references/critic-dispatch.md` per role; the adjudication checklist, ledger mechanics (`references/ledger.md`), and gate discipline (`references/review-duties.md`). If `bloks` responds on this machine, briefs carry verbatim knowledge cards and reports feed back ack/nack/learn: `references/knowledge-loop.md`.
3. **Verification** — goal-backward against must_haves + final whole-branch review read personally + iterative fresh-eyes rounds until 2 consecutive rounds clean of Critical/Important CODE findings (text-only and Minor findings never reset the counter; intermediate work is capped at 5 rounds, the final state is not: `references/verify-phase.md`). For user-facing behavior, offer a conversational UAT pass after the code-side gates (same file). After convergence: distill session lessons — recurring conventions go to the highest enforcement tier that can express them (lint > types > formatter > pre-commit > CI > prose; see `references/knowledge-loop.md`), the rest into persistent memory — knowledge must outlive the session.
4. **Branch finish** — after convergence, never stop silently and never integrate silently: if the base advanced past the branch's last verified merge point, re-gate on the merged head (full suite + the tail gates) BEFORE offering integration; then ONE integrate question — merge / PR / keep / discard — and the chosen option is EXECUTED and recorded, not just named. A post-convergence code fix re-opens convergence (a green suite alone never re-earns the question; a text-only fix does not). **Read `references/entry-and-finish.md` §2 ENTIRELY at every branch finish** — it owns the re-gate mechanics, the tail-gate list, and the re-open rules.
...
- Presented the integrate question after a post-convergence code fix on the strength of a green suite alone — the fix re-opened convergence (and a waived lineage owes its disposition ask); the stop criterion — or, on a waived lineage whose recorded disposition answer was extend, the fresh recorded waiver — must hold again first.

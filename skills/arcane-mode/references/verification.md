...
- **The workspace is single-writer:** one arcane/review session per checkout at a time — the ledger, briefs, and diff files have fixed paths; concurrent efforts need separate worktrees. A "stale" ledger might be a LIVE sibling session's — check for one before archiving.
- **The ledger belongs to ONE plan.** Its first line is the plan identity: `Plan: <plan file path> (<branch>)`. It is created when the plan is APPROVED (the identity line needs the plan file and branch) — before the premortem gate; plan-writing sessions correctly have no ledger yet. Skill start: read the ledger and compare its identity line against the current plan — a mismatch (new plan, new campaign phase) means the ledger is a previous effort's: archive it (`mv progress.md archive/<plan-slug>-progress.md`) and start a fresh one with the new identity line. Exception — the mismatch is merely a renamed/moved plan FILE of the same effort (same branch, same must_haves): update the identity line in place; archiving live completion marks would re-dispatch finished tasks. Completion marks are only ever trusted for the plan named in the identity line — never carried across plans.

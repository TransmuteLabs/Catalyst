...
2. **Class declared explicitly** — the prompt carries a `[dispatch-class:<id>]` marker naming the case this dispatch belongs to (execution `1a`/`1b`/`1c`/`1e`, or `critique`/`audit`/`analysis`/`scout`/`research`). Without it the dispatch falls into no case and is checked by nothing, so the gate DENIES it. WHICH class it is stays your decision — declaring it is not optional, and escalating after a failure means declaring a higher class. The agent's NAME must agree with the class: dispatching a critic under an executor class buys the executor's wider model set and is denied. Known ids and their model sets: `hooks/routing-table.toml`, also injected as a slice at session start.

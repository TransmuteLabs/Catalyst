...
Before submitting, the implementer verifies their own claims: created files exist (`[ -f path ]`), named commits exist (`git log`), the test baseline is exact (`N passed; 0 failed; K ignored`, per suite), AND the STATUS line matches the report body: any unresolved unexpected red mentioned ANYWHERE in the report (flags, concerns, "out-of-scope" sections) → the status must be one of the two sanctioned BLOCKED forms (own diff green / fix attempt failed) — a DONE/DONE_WITH_CONCERNS status above a flagged red fails this check. Result — a `Self-Check: PASSED/FAILED` section in the report. FAILED → don't submit, fix.

...
- **For a one-way door, build the candidates — don't just sketch them (arena)** (absorbed from a principle-skill stack, journal 0.8.6): when a novel decision is also hard to reverse (a public interface, a schema, a data-format commitment), sketching on paper is not enough — the design space is exhausted by BUILDING 2-3 candidates in parallel, each in its own worktree, then judging them on running code. The judging is done by a reviewer on a DIFFERENT model family from the one that produced the candidates (model diversity is the adversarial signal — a same-model judge subtly prefers its own shape), against the concrete rubric fixed BEFORE the candidates were built (so the bar is not drawn around the winner). The winner is taken as the base and the best parts of the losers are grafted in; the rejected alternatives and WHY each lost are recorded — the rejection notes are the highest-signal part of the record, the thing that stops the same dead shape being re-proposed later. If the candidates converge (all landed on nearly the same shape), that is the answer — ship the consensus, skip the graft; if they diverge wildly, the requirements were under-specified — reframe, do not average. This upgrades the sketch-only design-space lens above; reserve it for genuinely irreversible, novel ground (its cost is real), and only under the parallel-executor license the ledger's two-tier contract governs.

...
- Live databases are inspected with LAYERED read-only mechanics, not discipline alone: a read-only connection mode where the engine offers one (SQLite `mode=ro` URI, a read-only session on Postgres), statements restricted to SELECT/EXPLAIN/schema reads, the statement checked for write keywords AFTER stripping string literals (a literal containing "DROP" is data, not a command), and multi-statement input rejected.
- A prose-like file too large for one pass (Read truncates): split into overlapping chunks by size and read them in sequence (or ask the orchestrator to fan out one scout per chunk), each chunk answering the SAME question list; structured data (CSV/JSON/logs) is never chunk-read — query it with the right tool (jq, awk, sqlite) and quote the command as the coordinate.

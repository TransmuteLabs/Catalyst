...
def check_limits(sink, table):
    ...
    Причина отключения названа в таблице, а не здесь: демон отдал пул одним
    числом при нескольких аккаунтах вендора, а сколько аккаунтов он знает —
    НЕ ИЗМЕРЕНО. Агрегация по аккаунтам у самого гейта исправна (ряд lim8).
    """
        ...
        if not deny_enabled:
            print_warn("Dispatch gate: quota exhausted but deny is DISABLED by "
                       ...
                       + ". Отказ снят решением юзера 2026-09-21: у вендора "
                         "несколько аккаунтов, а демон назвал пул ОДНИМ "
                         "числом; сколько аккаунтов он знает — НЕ ИЗМЕРЕНО, "
                         "поэтому число пула не принимается за доступность "
                         "модели. Агрегация по аккаунтам у гейта исправна "
                         "и запинена рядом lim8.")
            return

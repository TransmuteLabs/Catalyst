...
describe("prompt.context (AGENTS.md carrier)", () => {
  type CtxCap = { ancestorCalls: any[]; nextEvent: any }
  ...
  function contextCap(
    ...
    const cap: CtxCap = { ancestorCalls: [], nextEvent: null }
    wired(on, now, env, {})
    on("fs.ancestors", (_$, e) => {
      cap.ancestorCalls.push(e)
      const value = typeof found === "function" ? (found as () => any)() : found
      return { value }
    })
    on("prompt.context", (_$, e) => {
      cap.nextEvent = e
      return { blocks: e.blocks, instructionFiles: e.instructionFiles }
    })
    return cap
  }

  test("carrier unset: next sees the same object and ancestors is not called", async ($, on) => {
    const cap = contextCap(on, 240_000_000, {}, [])
    const blocks = [{ name: "claudeMd", text: "b1" }]
    const instructionFiles = [{ path: "/work/CLAUDE.md", kind: "project", content: "c" }]
    const input = { blocks, instructionFiles }
    await $.prompt.context(input)
    expect(cap.nextEvent, "next received the input fields").toEqual(input)
    expect(cap.ancestorCalls, "fs.ancestors must not run when the carrier is not mod").toEqual([])
  })

  test("carrier mod, instructionFiles undefined: pass-through, ancestors not called", async ($, on) => {
    const cap = contextCap(on, 246_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, [])
    const blocks = [{ name: "claudeMd", text: "b2" }]
    const input = { blocks }
    await $.prompt.context(input)
    expect(cap.nextEvent.blocks).toEqual(blocks)
    expect(cap.nextEvent.instructionFiles).toBeUndefined()
    expect(cap.ancestorCalls).toEqual([])
  })

  test("carrier mod, no CLAUDE.md in dir: AGENTS.md is added as kind project", async ($, on) => {
    const found = [{
      dir: "/repo",
      name: "AGENTS.md",
      content: "agents-body",
      parts: [{ path: "/repo/AGENTS.md", content: "agents-body" }],
    }]
    const cap = contextCap(on, 252_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b3" }]
    const instructionFiles = [
      { path: "/elsewhere/CLAUDE.md", kind: "project", content: "other" },
    ]
    const r = await $.prompt.context({ blocks, instructionFiles })
    expect(cap.ancestorCalls).toHaveLength(1)
    expect(cap.ancestorCalls[0].names).toEqual(["AGENTS.md", ".claude/AGENTS.md"])
    const list = cap.nextEvent.instructionFiles
    expect(list, "list must not be voided").toBeDefined()
    const added = list.filter((f: any) => f.path === "/repo/AGENTS.md")
    expect(added, "the found AGENTS.md is present").toHaveLength(1)
    expect(added[0].kind).toBe("project")
    expect(added[0].content).toBe("agents-body")
    expect(r.instructionFiles, "host result also keeps the list").toBeDefined()
  })

  test("same-dir CLAUDE.md already present: AGENTS.md is not added", async ($, on) => {
    const found = [{
      dir: "/work",
      name: "AGENTS.md",
      content: "agents-body",
      parts: [{ path: "/work/AGENTS.md", content: "agents-body" }],
    }]
    const cap = contextCap(on, 258_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b4" }]
    const instructionFiles = [
      { path: "/work/CLAUDE.md", kind: "project", content: "claude-body" },
    ]
    await $.prompt.context({ blocks, instructionFiles })
    expect(cap.ancestorCalls).toHaveLength(1)
    const list = cap.nextEvent.instructionFiles
    expect(list).toHaveLength(1)
    expect(list.find((f: any) => f.path === "/work/AGENTS.md")).toBeUndefined()
  })

  test("candidate path already in the list: record count does not grow", async ($, on) => {
    const found = [{
      dir: "/repo",
      name: "AGENTS.md",
      content: "fresh",
      parts: [{ path: "/repo/AGENTS.md", content: "fresh" }],
    }]
    const cap = contextCap(on, 264_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b5" }]
    const instructionFiles = [
      { path: "/repo/AGENTS.md", kind: "project", content: "already" },
    ]
    await $.prompt.context({ blocks, instructionFiles })
    expect(cap.ancestorCalls).toHaveLength(1)
    expect(cap.nextEvent.instructionFiles.length, "path dedup keeps the count").toBe(1)
  })

  test("trimmed content matches an existing project record: not added", async ($, on) => {
    const found = [{
      dir: "/repo",
      name: "AGENTS.md",
      content: "  same-body  ",
      parts: [{ path: "/repo/AGENTS.md", content: "  same-body  " }],
    }]
    const cap = contextCap(on, 270_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b6" }]
    const instructionFiles = [
      { path: "/other/CLAUDE.md", kind: "project", content: "same-body" },
    ]
    await $.prompt.context({ blocks, instructionFiles })
    expect(cap.ancestorCalls).toHaveLength(1)
    expect(cap.nextEvent.instructionFiles.length, "content dedup keeps the count").toBe(1)
    expect(cap.nextEvent.instructionFiles.find((f: any) => f.path === "/repo/AGENTS.md")).toBeUndefined()
  })

  test("parts of length 2: the second record parent is the first part path", async ($, on) => {
    const found = [{
      dir: "/repo",
      name: "AGENTS.md",
      content: "main",
      parts: [
        { path: "/repo/AGENTS.md", content: "main" },
        { path: "/repo/imported.md", content: "imported" },
      ],
    }]
    const cap = contextCap(on, 276_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b7" }]
    const instructionFiles: any[] = []
    await $.prompt.context({ blocks, instructionFiles })
    const list = cap.nextEvent.instructionFiles
    expect(list).toHaveLength(2)
    const second = list.find((f: any) => f.path === "/repo/imported.md")
    expect(second).toBeDefined()
    expect(second.parent).toBe("/repo/AGENTS.md")
    const first = list.find((f: any) => f.path === "/repo/AGENTS.md")
    expect(first.parent).toBeUndefined()
  })

  test("insert after last project/local and before first memory", async ($, on) => {
    const found = [{
      dir: "/repo/sub",
      name: "AGENTS.md",
      content: "agents-body",
      parts: [{ path: "/repo/sub/AGENTS.md", content: "agents-body" }],
    }]
    const cap = contextCap(on, 282_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b8" }]
    const instructionFiles = [
      { path: "/repo/CLAUDE.md", kind: "project", content: "claude" },
      { path: "/mem/notes.md", kind: "memory", content: "mem" },
    ]
    await $.prompt.context({ blocks, instructionFiles })
    const paths = cap.nextEvent.instructionFiles.map((f: any) => f.path)
  ...
  test("fs.ancestors throws: next gets the original list, the throw does not escape", async ($, on) => {
    const cap: CtxCap = { ancestorCalls: [], nextEvent: null }
    wired(on, 288_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, {})
    on("fs.ancestors", (_$, e) => {
      cap.ancestorCalls.push(e)
      throw new Error("ancestors-boom")
    })
    on("prompt.context", (_$, e) => {
      cap.nextEvent = e
      return { blocks: e.blocks, instructionFiles: e.instructionFiles }
    })
    const blocks = [{ name: "claudeMd", text: "b9" }]
    const instructionFiles = [{ path: "/work/CLAUDE.md", kind: "project", content: "c" }]
    const input = { blocks, instructionFiles }
    let threw: any = null
    try {
    ...
    expect(threw, "the throw must not escape the handler").toBeNull()
    expect(cap.nextEvent).toEqual(input)
    expect(cap.nextEvent.instructionFiles).toEqual(instructionFiles)
  })

  test("blocks on the way out match the input by reference or value across teeth 3-8", async ($, on) => {
    const found = [{
      dir: "/repo",
      name: "AGENTS.md",
      content: "agents-body",
    ...
    const cap = contextCap(on, 294_000_000, { CLAUDE_MEMORY_CARRIER: "mod" }, found)
    const blocks = [{ name: "claudeMd", text: "b10" }]
    const instructionFiles = [
      { path: "/elsewhere/CLAUDE.md", kind: "project", content: "other" },
    ]
    const r = await $.prompt.context({ blocks, instructionFiles })
    expect(cap.nextEvent.instructionFiles, "files not voided: handler did not also rewrite blocks").toBeDefined()
    expect(Array.isArray(cap.nextEvent.instructionFiles)).toBe(true)
    expect(r.instructionFiles, "result files not voided").toBeDefined()
  })
})

...
**The dispatch gate.** Tiering that lives only in prose decays into "just send opus": naming a model is one parameter, routing a task to the right cheap-but-capable executor is a decision the orchestrator has to remember to make. So the rules are data, not advice. `hooks/routing-table.toml` holds them and a PreToolUse hook enforces them on every `Task`/`Agent` dispatch and on the envoy/proxy Bash channels; the same table renders the compact slice injected at session start, so there is no second copy to drift.

A breach REFUSES the dispatch. That is the point: a rule about which model runs a case is binding, and only a deny makes it so — advice that can be walked past is the failure mode this whole layer exists to fix. What is advisory is the fleet observation below, which is a different hook and never blocks. A non-blocking `[dispatch] mode = "warn"` exists for the rare period where the rules are deliberately suspended, and it costs exactly that bindingness; it is not the default. An unreadable table blocks in either mode: that is not a routing choice, it is the inability to check anything at all.

Every dispatch must name its model explicitly (an omitted one silently inherits the session's expensive top model) and declare its case with a `[dispatch-class:<id>]` marker — in the `Task`/`Agent` prompt, or in the command text on the vendor channels (envoy, a vendor CLI, the proxy), which are dispatches too — execution classes `1a`/`1b`/`1c`/`1e`, or `critique`/`audit`/`analysis`/`scout`/`research`/`adjudication`. A dispatch belonging to no case would be checked by nothing, which is the hole the gate exists to close; WHICH class it is stays the orchestrator's decision. `[classes.*]` is the sole home of model sets — allow-lists, matched exactly, so an unmeasured sibling (`gpt-5.6-luna` next to the measured `gpt-5.6-sol`) is a different model and does not pass. `[roles.*]` carries no second list: it maps an agent NAME to its class, so a critic dispatched under an executor class is caught rather than buying the wider set. Adjudication maps to an empty set — never delegated. Proxy models must carry an explicit effort at their measured pin, because a vendor's silent default effort once invalidated a whole arena ranking.

The same rules follow the models onto the Bash channels: the envoy companion, the proxy POST, the proxy-critique wrapper, and a vendor CLI launched DIRECTLY outside envoy — same models and same ledger, so the same effort and admission discipline, or the whole table would be one `codex exec` away from advisory. Only the executable position of a command counts, so `grep codex notes.md` is not a codex run.

The gate is fail-closed: a missing, unparseable, or unknown-schema table blocks dispatches with the cause named, and non-channel Bash exits before any table read so a broken table never blocks unrelated work. Overrides layer — plugin base, then `~/.claude/catalyst/routing-override.toml`, then `<project>/.claude/catalyst/routing-override.toml` (found by walking up from cwd) — merging by named entry, so a project retunes one class without restating the table. To try a model that has not been measured yet, one parameter opens the allow-lists (`[experiment] allow_all_models`); it never opens a case that is delegated to nobody, and never waives the explicit model, the effort, or the class declaration. Each rule is pinned by a mutant test: remove or invert it in the table and the verdict must flip, proving the rule lives in the table rather than in the enforcement code.

**Fleet statistics.** A gate can only judge the dispatch in front of it; it cannot see that the last eight went to the same model — and that is the actual field failure, because monoculture is invisible per-dispatch and obvious in aggregate. A PostToolUse hook keeps the aggregate per session — model, class, role, agent, channel — and judges it on two independent signals. **Dominance** catches one model taking the window. **Coverage** catches what dominance cannot see at all: a steady rotation between two or three models never crosses a share threshold while the rest of the fleet never runs. Ids of one model under two spellings collapse for counting, or the rotation reads as diversity that does not exist. The verdict is passed on the last N minutes rather than the whole session, because a session can run for a week and a model used five days ago would otherwise count as "the fleet is in use" while today is monoculture; the cumulative view stays in `--report` as context. The observation names the dominant model, the models that have not run at all, and the classes standing idle with their cheaper executors. It is a nudge, never a block: the dispatch already ran, and tasks genuinely of one class are a legitimate answer. Two brakes must both clear before it speaks — a dispatch counter against repeating inside a burst, and a clock against nagging through a slow session — because a reminder that arrives too often stops being read, and then it protects nothing. Concentration that is a DECISION rather than a lapse is named as such (`deliberate_model`), which silences the fleet observation while leaving the gate's rule reminders in force. Thresholds are table data (`[stats]`), so they retune and mutant-test like the rest. `hooks/dispatch-stats.py --report` prints the session's cumulative and recent tables. The recorder is deliberately separate from the gate, which provably writes nothing.

**Configuring it.** `hooks/routing-override.template.toml` is a commented, inert copy of every knob: copy it to `~/.claude/catalyst/routing-override.toml` or `<project>/.claude/catalyst/routing-override.toml` and uncomment what you actually override. One trap is called out there and worth repeating: merging is by NAMED ENTRY, so a redefined class replaces its base wholesale — list its models in full, they are not appended.
